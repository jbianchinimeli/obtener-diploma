package com.digitalhouse.obtenerdiploma.integration;
...
class ObtenerDiplomaApplicationTests {
    ...
    void analyzeNotesBadAverageTest() throws Exception {
        ...
        this.mockMvc.perform(post("/analyzeNotes").contentType(MediaType.APPLICATION_JSON).content(request))
                .andDo(print()).andExpect(status().isOk()).andExpect(jsonPath("$.average").value(7.8));
    }
    ...
    void analyzeNotesGoodAverageTest() throws Exception {
        String request = "{\"name\": \"Genghis Kan\", \"subjects\": [" +
                ...
                "{\"subject\":\"Matemática Superior\",\"note\": 10}" + "]" + "}";
        this.mockMvc.perform(post("/analyzeNotes").contentType(MediaType.APPLICATION_JSON).content(request))
                .andDo(print()).andExpect(status().isOk()).andExpect(jsonPath("$.average").value(9.2));
    }
    ...
    void analyzeNotesGoodMessageTest() throws Exception {
        ...
        this.mockMvc.perform(post("/analyzeNotes").contentType(MediaType.APPLICATION_JSON).content(request))
                ...
                jsonPath("$.message").value("¡Felicitaciones Genghis Kan! Usted tiene el gran promedio de 9.2"));
    }
}
